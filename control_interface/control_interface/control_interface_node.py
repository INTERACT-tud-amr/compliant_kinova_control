...
class ControlInterfaceNode:
    ...
    def __init__(self, args) -> None:
        ...
        rospy.Subscriber("bluetooth_teleop/joy", Joy, self.joystick_callback, queue_size=1)
        
        self._compliant = False
        self.automove_target = False
        self.state = State(self.simulate)
        self._joint_state = JointState()
        self._joint_state.name = [f'joint_{i}' for i in range(1, 7)]
    ...
    def joystick_callback(self, msg: Joy):
        cross_button = int(msg.buttons[0])
        circle_button = int(msg.buttons[1])
        square_button = int(msg.buttons[2])
        triangle_button = int(msg.buttons[3])
        arrow_left = int(msg.buttons[11])
        arrow_right = int(msg.buttons[12])
        if arrow_left == 1 and not self._compliant:
            self.make_compliant(Bool(data=True))
        if arrow_right == 1 and self._compliant:
            self.make_compliant(Bool(data=False))
    ...
    def make_compliant(self, msg: Bool):
        if msg.data: 
          ...
          self.kinova.pref()
          rospy.logwarn("Switching mode. Do not touch the robot")
          time.sleep(5)
          self.kinova.start_LLC()
          self.kinova.connect_LLC()
          self.state.controller.toggle('arm')
          rospy.logwarn("Switching successfull.")
        else:
          self._compliant = False
